...
export async function GET({ props }: { props: SectionProps }) {
  ...
  const displayLabel = label === "docs" ? "Artifact Server documentation" : label;

  const lines = [
    `# ${displayLabel}`,
    "",
    `> Clean Markdown pages for the ${displayLabel} section.`,
    "",
    "Use this index to select the smallest page that answers the current question.",
    "",
    "## Pages",
    "",
  ];
