...
Artifact Server 0.1.0 publishes a checksum file, SPDX SBOMs, and GitHub build attestations with its prerelease assets. Download and verify the portable Node package:

```sh
gh release download v0.1.0 \
  --repo plannotator/artifact-server \
  --pattern artifact-server-0.1.0-node.tar.gz \
  --pattern SHA256SUMS
grep ' artifact-server-0.1.0-node.tar.gz$' SHA256SUMS | sha256sum --check
gh attestation verify artifact-server-0.1.0-node.tar.gz \
  --owner plannotator \
  --deny-self-hosted-runners
```

The release also records the immutable container reference in `image-reference.txt`. Verify its provenance directly from GHCR:
...
The release workflow builds from the annotated tag, runs the complete release gate without a dependency cache, publishes the image by digest, checks every downloadable asset, and creates the prerelease only after the required jobs pass. The clean-install workflow then exercises the package on Linux and macOS and runs the immutable image through the compact Compose profile.
