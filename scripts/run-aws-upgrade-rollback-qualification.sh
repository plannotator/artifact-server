...
if [[ ! "$candidate_image" =~ @sha256:[a-f0-9]{64}$ ]]; then
  echo "ARTIFACT_SERVER_AWS_UPGRADE_IMAGE must be an immutable image digest reference." >&2
  exit 2
fi
...
ARTIFACT_SERVER_AWS_QUALIFICATION_EVIDENCE="$upgrade_evidence" \
  scripts/run-aws-deployment-product-qualification.sh

apply_image "$original_image"
original_restored=1
stack_output > "$rollback_file"
ARTIFACT_SERVER_AWS_QUALIFICATION_EVIDENCE="$rollback_evidence" \
  scripts/run-aws-deployment-product-qualification.sh
